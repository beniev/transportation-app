        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          {order.customer_email && (
            <div>
              ...
              <a href={`mailto:${order.customer_email}`} className="font-medium text-teal-600 hover:text-teal-800 hover:underline" dir="ltr">
                {order.customer_email}
              </a>
            </div>
          )}
          {order.customer_phone && (
            <div>
              ...
              <div className="flex items-center gap-2">
                <a href={`tel:${order.customer_phone}`} className="font-medium text-teal-600 hover:text-teal-800 hover:underline" dir="ltr">
                  {order.customer_phone}
                </a>
                <a
                  href={`https://wa.me/972${order.customer_phone.replace(/^0/, '')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium text-green-700 bg-green-100 rounded-full hover:bg-green-200 transition-colors"
                >
                  <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.121.553 4.115 1.519 5.845L.525 23.471l5.761-1.512A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818c-1.93 0-3.756-.51-5.33-1.398l-.382-.227-3.965 1.04 1.058-3.867-.249-.396A9.77 9.77 0 012.182 12c0-5.413 4.405-9.818 9.818-9.818S21.818 6.587 21.818 12 17.413 21.818 12 21.818z"/></svg>
                  WhatsApp
                </a>
              </div>
            </div>
